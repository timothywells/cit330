...
export default class QuakesController {
  ...
  constructor(parent, position = null) {
    ...
    this.area = document.querySelector('#radius').value;
  }
  ...
  async init() {
    ...
    await this.initPos();
    this.getQuakesByRadius(this.area);
  }
  ...
  async initPos() {
    ...
    if (this.position.lat === 0) {
      try {
        ...
        const thePosition = await getLocation();
        console.log(thePosition);
        // if we get the location back then set the latitude and longitude into this.position
        this.position.lat = thePosition.coords.latitude;
        this.position.lon = thePosition.coords.longitude;
